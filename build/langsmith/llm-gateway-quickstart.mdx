...
**Supported chat models — Python** (`langchain-core >= 0.3`)
...
**Supported chat models — TypeScript** (`@langchain/core >= 0.3`)
...
Provider-specific base URLs take precedence over the gateway, so you can still route an individual provider elsewhere. For example, with the gateway enabled, `OPENAI_API_BASE` (Python) or `OPENAI_BASE_URL` (TypeScript) sends OpenAI to that URL while every other provider continues to use the gateway:

```bash
export OPENAI_API_BASE="https://my.custom.gateway/openai/v2"   # Python
export OPENAI_BASE_URL="https://my.custom.gateway/openai/v2"   # TypeScript
```

The following table shows how the base URL and key are resolved, using OpenAI as the example (other providers use their own `*_API_BASE` / `*_BASE_URL` and `*_API_KEY` variables). `GW default` is `https://gateway.smith.langchain.com/openai/v1`.
